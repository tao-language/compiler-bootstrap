...
> **Last updated:** 2026-05-11 (Phase 2 — 939 tests passing, 9 failures. Phase 3 — In progress.)
...
>
> **Recent:** Haskell-style application parsing implemented: `f x y z` instead of `f(x, y, z)`. Fixed Hole(-1) for uninstantiated holes in FFI calls. Added PLitT pattern type for literal types. Updated term_to_string for new syntax. 939 tests passing, 9 failures (Tour tests: factorial, implicit params, guards).
>
...
| Date | Change |
|------|--------|
| 2026-05-11 | **Haskell-style application parsing + Hole(-1) fix.** Replaced `parse_term_with_app`/`parse_app_chain` with new `parse_app` function that handles Haskell-style space-separated application (`f x y z` instead of `f(x, y, z)`). Fixed FFI call argument types to use `Hole(-1)` instead of `Hole(0)` for uninstantiated holes, allowing type inference to assign different types to each argument. Added `PLitT` pattern type for matching literal types (`$Int`, `$Float`, etc.). Updated `term_to_string` to use new syntax: `(fun arg)` for App, `%name<type>(args)` for Call. Updated all Tour files and tests to use new syntax. **939 tests passing, 9 failures** (Tour tests: factorial, implicit params, guards need further debugging). |
