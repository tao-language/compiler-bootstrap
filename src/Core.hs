...
reduce env (Fun (Ann p a) b) = Fun (Ann p (reduce env a)) (reduce env b)
...
reduce env (For x a) = reduce ((x, Var x) : env) a
...
eval env term = case reduce env term of
  ...
  Fun (Ann p a) b -> Fun (Ann p (reduce env a)) (reduce env b)
