...
taoLangTests = describe "--==☯ Tao language ☯==--" $ do
  ...
  it "☯ expression" $ do
    ...
    p "x = 1\na" `shouldBe` Right (Let [Untyped "x" i1] a, "")
    p "x = 1; y = 2; a" `shouldBe` Right (Let [Untyped "x" i1, Untyped "y" i2] a, "")
    p "x = 1\ny = 2\na" `shouldBe` Right (Let [Untyped "x" i1, Untyped "y" i2] a, "")
