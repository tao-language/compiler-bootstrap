...
run = describe "--== Core precedence ==--" $ do
  ...
  let prec' :: String -> Either String (Expr, String)
      prec' src = case parse 0 "<CorePrecedence>" src of
        ...
        Left s -> Left ("parse error, remaining: " ++ s.remaining)
  let prec :: String -> Either String Expr
      prec src = do
        (a, out) <- prec' src
        if src == out then Right a else Left ("format error, expected: " ++ src ++ ", got: " ++ out)
  ...
  it "☯ CorePrecedence.Fun" $ do
    prec' "x -> y | z" `shouldBe` Right (x `Fun` y `Or` z, "| x -> y\n| z")
    prec "x -> @y. z" `shouldBe` Right (Fun x (for ["y"] z))
    prec' "x -> y : z" `shouldBe` Right (x `Fun` y `Ann` z, "x -> y\n: z")
    prec' "x -> y -> z" `shouldBe` Right (x `Fun` (y `Fun` z), "x ->\n  y -> z")
