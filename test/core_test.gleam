...
pub fn instantiate_ctr_arg_undefined_test() {
  ...
  c.instantiate_ctr(0, [], "A", ctr_def, v32t, s2)
  // |> should.equal(#([], [#("x", #(c.VErr(c.VarUndefined(0, s)), None))], v32t, []))
  |> should.equal(todo)
}
...
pub fn instantiate_ctr_arg_unsolved_test() {
  ...
  c.instantiate_ctr(0, [], "A", ctr_def, v32t, s2)
  // |> should.equal( #( [c.VNeut(c.HMeta(0), [])], [#("x", #(c.VNeut(c.HMeta(0), []), None))], v32t, [ c.CtrUnsolvedParam("A", ctr_def, 0, s2), ],),)
  |> should.equal(todo)
}
...
pub fn ctr_check_arg_mismatch_test() {
  let tenv = [#("A", c.CtrDef([], i32t, i64t))]
  c.check(0, [], tenv, ctr("A", i64(1)), v64t, s2)
  |> should.equal(c.VBad(v64t, [c.TypeMismatch(v32t, v64t, s, s2)]))
}

pub fn ctr_check_ret_mismatch_test() {
  let tenv = [#("A", c.CtrDef([], i32t, i64t))]
  c.check(0, [], tenv, ctr("A", i32(1)), v32t, s2)
  |> should.equal(c.VBad(v64t, [c.TypeMismatch(v64t, v32t, s, s2)]))
}

// --- Rcd --- \\
pub fn rcd_eval_test() {
  c.eval([], rcd([])) |> should.equal(c.VRcd([]))
  c.eval([], rcd([#("a", i32t)])) |> should.equal(c.VRcd([#("a", v32t)]))
  c.eval([], rcd([#("a", i32t), #("b", i64t)]))
  |> should.equal(c.VRcd([#("a", v32t), #("b", v64t)]))
}

pub fn rcd_unify_missing_test() {
  let ab = c.VRcd([#("a", v32t), #("b", v64t)])
  c.unify(0, [], ab, c.VRcd([]), s, s2)
  |> should.equal(Error(c.RcdMissingFields(["a", "b"], s2)))
  c.unify(0, [], c.VRcd([]), ab, s, s2)
  |> should.equal(Error(c.RcdMissingFields(["a", "b"], s)))
}

pub fn rcd_unify_mismatch_test() {
  c.unify(0, [], c.VRcd([#("a", v32t)]), c.VRcd([#("a", v64t)]), s, s2)
...
pub fn dot_eval_test() {
  ...
  c.eval([], dot(rcd([]), "a"))
  |> should.equal(c.VErr(c.DotFieldNotFound("a", [], s)))
...
pub fn dot_infer_record_test() {
  c.infer(0, [], [], dot(rcd([#("x", i32(1)), #("y", i64(2))]), "x"))
  |> should.equal(v32t)

  c.infer(0, [], [], dot(rcd([#("x", i32(1)), #("y", i64(2))]), "y"))
  |> should.equal(v64t)
}

pub fn dot_infer_record_not_found_test() {
  c.infer(0, [], [], dot(rcd([]), "x"))
  |> should.equal(c.VErr(c.DotFieldNotFound("x", [], s)))
}

pub fn dot_check_test() {
  let ab = rcd([#("a", i32(1)), #("b", i64(2))])
  c.check(0, [], [], dot(ab, "a"), v32t, s2)
  |> should.equal(v32t)
  c.check(0, [], [], dot(ab, "b"), v64t, s2)
  |> should.equal(v64t)
}
...
pub fn bind_pattern_ctr_undefined_test() {
  let p = c.PCtr("A", c.PAny)
  c.bind_pattern(0, [], [], p, v32t, s, s2)
  |> should.equal(#(0, [], [c.PatternMismatch(p, v32t, s, s2)]))
  c.bind_pattern(0, [], [], p, c.VCtr("A", v32t), s, s2)
  |> should.equal(#(0, [], []))
  c.bind_pattern(0, [], [], p, c.VCtr("B", v32t), s, s2)
  |> should.equal(#(0, [], [c.PatternMismatch(p, c.VCtr("B", v32t), s, s2)]))
}
...
pub fn bind_pattern_rcd_missing_test() {
  let p = c.PRcd([#("a", pvar("x")), #("b", pvar("y"))])
  let t = c.VRcd([])
  c.bind_pattern(0, [], [], p, t, s, s2)
  // |> should.equal( #(2, [#("y", hole(1)), #("x", hole(0))], [c.CtrMissingArgs(["a", "b"], s2)]),)
  |> should.equal(todo)
}
