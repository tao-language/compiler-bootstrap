...
run = describe "--==☯️ Core language ☯️==--" $ do
  ...
  it "☯ infer Maybe" $ do
    ...
    infer' (just i1) `shouldBe` Right (maybe IntT)
  ...
  it "☯ infer Vec" $ do
    ...
    infer' (cons [Num 1.1, nil]) `shouldBe` Right (vec [i1, NumT])
    infer' (cons [Num 1.1, cons [Num 2.2, nil]]) `shouldBe` Right (vec [i2, NumT])
