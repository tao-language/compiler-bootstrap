...
coreTests = describe "--== Core tests ==--" $ do
  ...
  it "☯ compileCases" $ do
    ...
    compileCases ctx [([PAny], Int 1)] `shouldBe` Just (L.Lam "%0" (L.Int 1))
    ...
    compileCases ctx [([PCtr "A" []], Int 1), ([PCtr "B" [x_]], Int 2)] `shouldBe` Just (L.Lam "%0" (L.app (L.Var "%0") [L.Int 1, L.Lam "x" (L.Int 2)]))
    compileCases ctx [([PCtr "A" [], x_], Int 1), ([PCtr "B" [y_], z_], Int 2)] `shouldBe` Just (L.Lam "%0" (L.app (L.Var "%0") [L.Lam "x" (L.Int 1), L.lam ["y", "z"] (L.Int 2)]))
    compileCases ctx [([PAnn x_ y_], Int 1)] `shouldBe` Just (L.lam ["x", "y"] (L.Int 1))
    compileCases ctx [([PAnn (PInt 0) x_], Int 1), ([y_], Int 2)] `shouldBe` Just (L.Lam "y" (L.app (L.eq (L.Var "y") (L.Int 0)) [L.Lam "x" (L.Int 1), L.Lam "%0" (L.Int 2)]))
