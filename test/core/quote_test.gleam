...
pub fn quote_var_test() {
  ...
  quote(c.ffi_build, 1, ast.VNeut(ast.HVar(0), []), s1)
  |> should.equal(ast.Var(0, s1))

  // At level 2, HVar(0) should become Var(1)
  quote(c.ffi_build, 2, ast.VNeut(ast.HVar(0), []), s1)
  |> should.equal(ast.Var(1, s1))

  // At level 2, HVar(1) should become Var(0)
  quote(c.ffi_build, 2, ast.VNeut(ast.HVar(1), []), s1)
  |> should.equal(ast.Var(0, s1))
}

pub fn quote_hole_test() {
  quote(c.ffi_build, 0, ast.VNeut(ast.HHole(0), []), s1)
  |> should.equal(ast.Hole(0, s1))
  quote(c.ffi_build, 0, ast.VNeut(ast.HHole(5), []), s1)
...
pub fn quote_verr_test() {
  quote(c.ffi_build, 0, ast.VErr, s1)
