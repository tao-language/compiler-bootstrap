...
const s0 = Span("exhaustiveness_test", 0, 0, 0, 0)

fn p_any() { ast.PAny }
fn p_unit() { ast.PUnit }
fn p_ctr(tag, inner) { ast.PCtr(tag, inner) }
fn p_lit(n) { ast.PLit(ast.I32(n)) }
fn p_rcd(fields) { ast.PRcd(fields) }
fn p_as(inner, name) { ast.PAs(inner, name) }
...
pub fn check_empty_patterns_returns_missing_test() {
  let errors = exhaustiveness.check_exhaustiveness([], [], s0)
...
pub fn check_identical_cases_detect_redundancy_test() {
  ...
  let errors = exhaustiveness.check_exhaustiveness([p1, p2], [], s0)
  is_len(errors, 1) |> should.be_true
}
...
pub fn check_wildcard_makes_all_redundant_test() {
  ...
  let errors = exhaustiveness.check_exhaustiveness([p1, p2, p3], [], s0)
...
pub fn check_wildcard_no_missing_cases_test() {
  let p1 = make_single(p_any())
  let errors = exhaustiveness.check_exhaustiveness([p1], [], s0)
  is_len(errors, 0) |> should.be_true
}
