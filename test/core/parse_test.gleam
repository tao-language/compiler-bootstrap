...
pub fn lex_typ_test() {
  ...
  assert lex("%Type\n<\n42\n>\n")
    == Ok([p.KwType, p.LAngle, p.IntLit(42), p.RAngle])
  assert lex("%Types ")
    == Error(e.Error(e.UnexpectedToken("%Types "), s(1, 1, 1, 1), []))
  assert lex("% Type ")
    == Error(e.Error(e.UnexpectedToken("% Type "), s(1, 1, 1, 1), []))
}
...
pub fn lex_ann_test() {
  assert lex("%(x:y)")
    == Ok([p.AnnOpen, p.Name("x"), p.Colon, p.Name("y"), p.RParen])
  assert lex("%( x : y ) ")
    == Ok([p.AnnOpen, p.Name("x"), p.Colon, p.Name("y"), p.RParen])
}

pub fn parse_ann_test() {
  assert parse("%(x : y)")
    == Ok(ast.ann(
      ast.var("x", s(1, 1, 1, 4)),
      ast.var("y", s(1, 5, 1, 8)),
...
// ============================================================================
pub fn lex_fix_test() {
  assert lex("%fix") == Ok([p.KwFix])
}

pub fn parse_fix_test() {
  assert parse("%fix f. x")
    == Ok(ast.fix("f", ast.var("x", s(1, 7, 1, 10)), s(1, 1, 1, 10)))
}
