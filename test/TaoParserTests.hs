...
run = describe "--==☯ TaoParser ☯==--" $ do
  ...
  it "☯ parseName" $ do
    ...
    p "Type abc" `shouldBe` Right (TaoKind, "abc")
    p "Int abc" `shouldBe` Right (TaoIntType, "abc")
    p "Num abc" `shouldBe` Right (TaoNumType, "abc")
    p "var abc" `shouldBe` Right (TaoVar "var", "abc")
    p "Tag abc" `shouldBe` Right (TaoTag "Tag", "abc")
  ...
  it "☯ parseExpr'" $ do
    ...
    p "x :  y" `shouldBe` Right (TaoAnn (var 1 1 "x") (var 1 6 "y"), "")
    p "x : @a. y" `shouldBe` Right (TaoAnn (var 1 1 "x") (TaoForAll "a" $ var 1 9 "y"), "")
    p "x : @a b. y" `shouldBe` Right (TaoAnn (var 1 1 "x") (taoForAll ["a", "b"] $ var 1 11 "y"), "")
  ...
  it "☯ parseDefinition" $ do
    ...
    p "x : a = y" `shouldBe` Right (([], TaoAnn (var 1 1 "x") (var 1 5 "a"), var 1 9 "y"), "")
  ...
  it "☯ parseTest" $ do
    ...
    p "> x : a" `shouldBe` Right (TaoTest (TaoAnn (var 1 3 "x") (var 1 7 "a")) (var 1 3 "x"), "")
