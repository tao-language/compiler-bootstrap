...
run = describe "--== Tao precedence ==--" $ do
  ...
  it "☯ TaoPrecedence.Op2.And" $ do
    ...
    prec "x and y // z" `shouldBe` Right (x `andOp` (y `divI` z))
  ...
  it "☯ TaoPrecedence.Op2.Or" $ do
    ...
    prec "x or y // z" `shouldBe` Right (x `orOp` (y `divI` z))
  ...
  it "☯ TaoPrecedence.Op2.Xor" $ do
    ...
    prec "x xor y // z" `shouldBe` Right (x `xorOp` (y `divI` z))
  ...
  it "☯ TaoPrecedence.Op2.DivI" $ do
    ...
    prec "x // y or z" `shouldBe` Right ((x `divI` y) `orOp` z)
    prec "x // y xor z" `shouldBe` Right ((x `divI` y) `xorOp` z)
    prec "x // y and z" `shouldBe` Right ((x `divI` y) `andOp` z)
