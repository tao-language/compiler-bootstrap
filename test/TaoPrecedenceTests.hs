...
run = describe "--== Tao precedence ==--" $ do
  ...
  it "☯ TaoPrecedence.Let" $ do
    ...
    prec "let x = y if z\na" `shouldBe` Right (Let (x, y `If` z) a)
  ...
  it "☯ TaoPrecedence.Or" $ do
    ...
    prec "x | y if z" `shouldBe` Right (x `Or` (y `If` z))
  ...
  it "☯ TaoPrecedence.If" $ do
    prec "x if y | z" `shouldBe` Right ((x `If` y) `Or` z)
    prec "x if y : z" `shouldBe` Right ((x `If` y) `Ann` z)
    ...
    prec "x if y xor z" `shouldBe` Right (x `If` (y `xorOp` z))
    prec "x if y and z" `shouldBe` Right (x `If` (y `andOp` z))
    prec "x if y == z" `shouldBe` Right (x `If` (y `eq` z))
    prec "x if y != z" `shouldBe` Right (x `If` (y `ne` z))
    prec "x if y < z" `shouldBe` Right (x `If` (y `lt` z))
    prec "x if y <= z" `shouldBe` Right (x `If` (y `le` z))
    prec "x if y > z" `shouldBe` Right (x `If` (y `gt` z))
    prec "x if y >= z" `shouldBe` Right (x `If` (y `ge` z))
    prec "x if y :: z" `shouldBe` Right (x `If` (y `cons` z))
    prec "x if @y. z" `shouldBe` Right (x `If` For ["y"] z)
    prec "x if y + z" `shouldBe` Right (x `If` (y `add` z))
    prec "x if y - z" `shouldBe` Right (x `If` (y `sub` z))
    prec "x if y * z" `shouldBe` Right (x `If` (y `mul` z))
    prec "x if y / z" `shouldBe` Right (x `If` (y `div'` z))
    prec "x if y // z" `shouldBe` Right (x `If` (y `div2` z))
    prec "x if y ^ z" `shouldBe` Right (x `If` (y `pow` z))
    prec "x if -y" `shouldBe` Right (x `If` neg y)
    prec "x if y(z)" `shouldBe` Right (x `If` (y `app1` z))
  ...
  it "☯ TaoPrecedence.Op2.And" $ do
    ...
    prec "x and y if z" `shouldBe` Right ((x `andOp` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Eq" $ do
    ...
    prec "x == y if z" `shouldBe` Right ((x `eq` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Ne" $ do
    ...
    prec "x != y if z" `shouldBe` Right ((x `ne` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Lt" $ do
    ...
    prec "x < y if z" `shouldBe` Right ((x `lt` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Le" $ do
    ...
    prec "x <= y if z" `shouldBe` Right ((x `le` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Gt" $ do
    ...
    prec "x > y if z" `shouldBe` Right ((x `gt` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Ge" $ do
    ...
    prec "x >= y if z" `shouldBe` Right ((x `ge` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Cons" $ do
    ...
    prec "x :: y if z" `shouldBe` Right ((x `cons` y) `If` z)
  ...
  it "☯ TaoPrecedence.Ann" $ do
    ...
    prec "x : y if z" `shouldBe` Right (x `Ann` (y `If` z))
  ...
  it "☯ TaoPrecedence.For" $ do
    ...
    prec "@x. y if z" `shouldBe` Right (For ["x"] y `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Add" $ do
    ...
    prec "x + y if z" `shouldBe` Right ((x `add` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Sub" $ do
    ...
    prec "x - y if z" `shouldBe` Right ((x `sub` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Mul" $ do
    ...
    prec "x * y if z" `shouldBe` Right ((x `mul` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Div" $ do
    ...
    prec "x / y if z" `shouldBe` Right ((x `div'` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.DivI" $ do
    ...
    prec "x // y if z" `shouldBe` Right ((x `div2` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op2.Pow" $ do
    ...
    prec "x ^ y if z" `shouldBe` Right ((x `pow` y) `If` z)
  ...
  it "☯ TaoPrecedence.Op1.Neg" $ do
    ...
    prec "-x if y" `shouldBe` Right (neg x `If` y)
  ...
  it "☯ TaoPrecedence.App" $ do
    ...
    prec "x(y) if z" `shouldBe` Right ((x `app1` y) `If` z)
