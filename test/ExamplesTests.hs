...
run = describe "--==☯ Examples ☯==--" $ do
  ...
  it ("☯ " ++ name ++ ".tao") $ do
    ...
    let testResults =
          [ TestPass name "Pass",
            TestFail name "Fail" (i1, i2) i1' i1,
            TestPass name "Shortcut pass",
            TestFail name "Shortcut fail" (i1, i2) i1' i1
          ]
  ...
  it ("☯ " ++ name ++ ".tao") $ do
    ...
    let testResults =
          [ TestPass name "Any match",
            ...
            TestFail name "Unit match fail" (Unit, i1) C.Unit Unit,
            TestPass name "IntT match",
            TestFail name "IntT match fail" (IntT, NumT) C.IntT IntT,
    ...
    let testResults =
          [ TestPass name "Call match",
            let p = C.Ann (C.Call "f" [y']) (C.Call "f" [yT'])
                ...
                core = C.Let [("y", C.For "yT" $ C.def ["y"] (p, a) y')] y'
             in TestFail name "Call match fail" (y, i1) core Err
          ]
    testAll [] pkg `shouldBe` testResults
